...
library OptoLib {
    ...
    "let fA=args[0];const r = async () => {try {const re=await Functions.makeHttpRequest({url: 'https://arbitrum-one-rpc.publicnode.com',method: 'POST',headers:{'Content-Type': 'application/json',},data:{id:1,jsonrpc: '2.0',method: 'eth_call',params: [{to: fA,data: '0x50d25bcd',},'latest',],},});const up=re.data.result;return up;}catch(error){console.error('Error occurred:', error);throw error;}};let up;try{const result=await r();up=Math.round(result/10**2);console.log(up);}catch(error){console.error('Failed to get balance:', error);}return Functions.encodeUint256(BigInt(up));";
    // Dynamic String for gas price 1 arg
    string public constant SUBGRAPH_QUERY_1 = 
    "let subgraphURL=args[0];const fetchPrice=()=>Functions.makeHttpRequest({url: subgraphURL,method: 'POST',headers:{'Accept': 'application/json','Content-Type': 'application/json',},data:{query: `query MyQuery {feeAggregators{gas_average_daily}}`,},});const {error,data:{data,},}=await fetchPrice();const {feeAggregators:[{ gas_average_daily }]}=data;const request=async()=>{try {const response=await Functions.makeHttpRequest({url: 'https://arbitrum-one-rpc.publicnode.com',method: 'POST',headers:{'Content-Type': 'application/json',},data:{id: 1,jsonrpc: '2.0',method: 'eth_call',params:[{to: '0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612',data:'0x50d25bcd',},'latest',],},});const balance=response.data.result;return balance;}catch(error){console.error('Error occurred:', error);throw error;}};let ethUsdcPrice;try{const result=await request();ethUsdcPrice=(parseInt(result, 16)/10**8);console.log(ethUsdcPrice);}catch(error){console.error('Failed to get balance:', error);}let parsedAvg=parseInt(gas_average_daily);let usdc_gas_average_daily=(parsedAvg*ethUsdcPrice)/1e12;let rounded_usdc_gas_average_daily = Math.floor(usdc_gas_average_daily);return Functions.encodeUint256(BigInt(rounded_usdc_gas_average_daily));";
    // Eth specific call for blob price - 0 args
    string public constant SUBGRAPH_QUERY_2 = 
    "const fetchPrice = () => Functions.makeHttpRequest({url: 'https://api.studio.thegraph.com/query/73482/opto-basefees-ethereum/version/latest',method: 'POST',headers: {'Accept': 'application/json','Content-Type': 'application/json',},data: {query: `query MyQuery {feeAggregators{blob_average_daily}}`,},});const {error,data:{data,},} = await fetchPrice();const {feeAggregators: [{ blob_average_daily }]} = data;const request = async () => {try {const response = await Functions.makeHttpRequest({url: 'https://arbitrum-one-rpc.publicnode.com',method: 'POST',headers: {'Content-Type': 'application/json',},data: {id: 1,jsonrpc: '2.0',method: 'eth_call',params: [{to: '0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612',data: '0x50d25bcd', },'latest',],},});const balance = response.data.result;return balance;} catch (error) {console.error('Error occurred:', error);throw error;}};let ethUsdcPrice;try {const result = await request();ethUsdcPrice = (parseInt(result, 16) / 10**8);console.log(ethUsdcPrice);} catch (error){console.error('Failed to get balance:', error);}let parsedAvg = parseInt(blob_average_daily);let usdc_blob_average_daily = (parsedAvg * ethUsdcPrice) / 1e12;let rounded_usdc_blob_average_daily = Math.floor(usdc_blob_average_daily);return Functions.encodeUint256(BigInt(rounded_usdc_blob_average_daily));";
