...
contract Opto is IOpto, ERC1155, FunctionsClient, AutomationCompatibleInterface, ConfirmedOwner, OptoUtils{ 
    ...
    function createOption(
        ...
        // Validate parameters
        require(premium > 0, "Opto: premium can't be 0");
        require(strikePrice > 0, "Opto: strike price can't be 0");
        require(
            buyDeadline > block.timestamp,
            "Opto: invalid buy deadline"
        );
        require(
            expirationDate > buyDeadline,
            "Opto: invalid expiration date"
        );
        require(units > 0, "Opto: invalid unit amount");
        require(capPerUnit > 0, "Opto: invalid cap per unit");
    ...
    function createCustomOption(
        ...
        require(premium > 0, "Opto: premium can't be 0");
        require(strikePrice > 0, "Opto: strike price can't be 0");
        require(buyDeadline > block.timestamp, "Opto: invalid buy deadline");
        require(expirationDate > buyDeadline, "Opto: invalid expiration date");
        require(units > 0, "Opto: invalid unit amount");
        require(capPerUnit > 0, "Opto: invalid cap per unit");
        // Calculate total collateral from the writer
        uint256 collateral = capPerUnit * units;
        // Transfer collateral from the writer to the contract
        require(IERC20(usdcAddress).transferFrom(msg.sender, address(this), collateral), "Opto: failed to transfer usdc");
        // Update lastOptionInd
        lastOptionId += 1;
        // Create option    
        options[lastOptionId] = Option(
    ...
    function buyOption(uint256 id, uint256 units) public {
        ...
        require(option.writer != address(0), "Opto: option id dosen't exist");
        // Check if option is paused
        require(!isPaused(option.statuses), "Opto: option paused");
        // Check if option is not expired
        require(
            block.timestamp < option.buyDeadline,
            "Opto: can't buy option"
        );
        // Check if there are enough units left
        require(option.unitsLeft >= units, "Opto: not enough units");
    ...
    function claimOption(uint256 id) public {
        ...
        require(block.timestamp >= option.expirationDate, "Opto: option is expired");
        // Check if option is deactived
        require(!isActive(option.statuses), "Opto: option is not active");
        // Check if option is 
        require(hasToPay(option.statuses), "Opto: option isnt solvible");
        // Check if the buyer has enough units
        require(balanceOf(msg.sender, id) >= units, "Opto: not enough units");
        ...
        require(IERC20(usdcAddress).transfer(msg.sender, price), "Opto: failed to transfer usdc");
    ...
    function claimForPausedOption(uint256 id, uint256 units, bool isWriter) public returns(uint256) {
        ...
        require(hasToPay(option.statuses), "Opto: option isnt solvible");
        // Check if option is paused
        require(isPaused(option.statuses), "Opto: option isnt solvible");
        // define price var
        uint256 price;
        // Check if for writer
        if (isWriter) {
            // Check if the caller is the writer
            require(msg.sender == option.writer, "Opto: already initialized");
            // Calculate total collateral
            price = (option.capPerUnit - option.premium) * units;
            // check price is greater than 0
            require(price > 0, "Opto: option isnt solvible");
    ...
    function deleteOption(uint256 id) public {
        ...
        Option storage option = options[id];
        // Check if option is deactived
        require(!isActive(option.statuses), "Opto: option activated");
        // Check if option is not already claimed
        require(!hasToPay(option.statuses), "Opto: ongoing option");
        // Transfer collateral from the contract to the writer
        require(
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if (op == 1){
            // check optionId
            require(optionId != 0, "Opto: invalid option id");
            // Get option from storage
    ...
    function _invokeSendRequest(
        ...
        if (queryId == 3) {
            // Custom query
            query = customOptionQueries[optionId];
            args = customOptionArgs[optionId];
        } else {
